...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('/accounts/login/?command=verification&email='+message+'&to_email='+to_email,context)
    else:    
...
def forgotPassword(request):
    if request.method == "POST":
        ...
        if Account.objects.filter(email=email).exists():
            ...
            return redirect('/accounts/login/?command=reset&email='+message+'&to_email='+to_email,context)
            # return redirect(login)
...
def order_detail(request, order_id):
    ...
    for i in order_detail:
        subotal+=i.product_price*i.quantity
    tax=str(order.tax)
    print('tax: ',order.tax)
    context={
        'order': order,
